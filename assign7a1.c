...
int main(void) //main method
{
	...
	if(rows==columns) //if matrix is a square one
	{
		...
		if(detM!=0) //invertible case
		{
			...
			float invMatrix[rows][columns]; //creation of inverse matrix
			for(i=0;i<rows;i++)
			{
				for(j=0;j<columns;j++)
					invMatrix[i][j]=adjM[i][j]*1.0/detM;
			}
			...
			for(i=0;i<rows;i++)
			{
				for(j=0;j<columns;j++)
				{
					for(k=2;k>1;k++) //printing in fractional notation
					{
						if(invMatrix[i][j]<0)
							flag=1;
						float num;
						if(flag==1)
							num=-1*invMatrix[i][j]*k;
						else
							num=invMatrix[i][j]*k;
					...
					{
						if(flag==1)
							printf("%5d",(-1*numerator)/denominator);
						else
							printf("%5d",numerator/denominator);
						continue;
					}
					
					if(flag==1)
						printf("%5d/%d",(-1*numerator),denominator);
					else
						printf("%5d/%d",numerator,denominator);
				}
				puts("");
			}
